...
class TestBasler(unittest.TestCase):
    ...
    def test_set_woi_width_above_range(self):
        ...
        self.assertIn(
            "Value = 2000 must be equal or smaller than Max = 1936. : OutOfRangeException thrown in node 'Width' while calling 'Width.SetValue()' (file 'IntegerT.h', line 79)",
            str(ctx.exception),
        )
    ...
    def test_set_woi_height_below_range(self):
        ...
        self.assertIn(
            "Value = 2000 must be equal or smaller than Max = 1216. : OutOfRangeException thrown in node 'Height' while calling 'Height.SetValue()' (file 'IntegerT.h', line 79)",
            str(ctx.exception),
        )
        self.assertEqual(max_woi, self.cam.woi)
